...
class FanPrivacyPolicyVC: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate  {

    let fanfinity_PrivacyUrl = "https://www.termsfeed.com/live/c1ff734a-6a1f-4574-bc99-ca79b400cf98"
    ...
    override func viewDidLoad() {
        ...
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.fanfinityGetUserDefaultKey())
        initSubViews()
        initNavView()
        fanfinityInitWebView()
        fanfinityStartLoadWebView()
    }
    ...
    private func fanfinityInitWebView() {
    ...
    private func fanfinityStartLoadWebView() {
        let urlStr = url ?? fanfinity_PrivacyUrl
    ...
    private func tlReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "FanPrivacyPolicyVC") as? FanPrivacyPolicyVC {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                fanfinitySendEvent(tName, values: jsonObject)
                                return
                            }
                        ...
                        fanfinitySendEvent(tName, values: [tName: data])
                    }
                } else {
                    fanfinitySendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                fanfinityAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    fanfinityAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = fanfinityJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = fanfinityJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    fanfinitySendEvents(withParams: evParams)
                }
            }
        }
    }
